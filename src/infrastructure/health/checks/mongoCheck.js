...
async function checkMongo(options = {}) {
  ...
  if (connection.readyState !== 1)
    return {
      status: "unhealthy",
      reason: `connection state is ${connection.readyState}`,
    };
  if (!options.ping) return { status: "healthy" };
  const admin = connection.db?.admin?.();
  const ping = admin?.ping?.bind(admin);
  if (typeof ping !== "function")
    return { status: "unhealthy", reason: "MongoDB ping is unavailable" };
  try {
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () =>
            reject(
              new HealthCheckError("MongoDB ping timed out", { timeoutMs }),
            ),
          timeoutMs,
        ),
      ),
