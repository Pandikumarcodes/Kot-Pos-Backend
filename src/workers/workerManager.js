...
function createWorkerManager({ queues, emailProvider, reportService, reportRenderer = async () => null, connection } = {}) {
  const workers = [
    createWorker(QUEUE_NAMES.EMAIL, processorFor(emailJobHandlers({ emailProvider })), { connection, concurrency: 5 }),
    createWorker(QUEUE_NAMES.INVENTORY_ALERTS, processorFor(inventoryJobHandlers({ emailProvider })), { connection, concurrency: 2 }),
    createWorker(QUEUE_NAMES.REPORTS, processorFor(reportJobHandlers({ reportService, reportRenderer, emailProvider })), { connection, concurrency: 1 }),
    createWorker(QUEUE_NAMES.CLEANUP, processorFor(cleanupJobHandlers({ queues })), { connection, concurrency: 1 }),
  ].filter(Boolean);
