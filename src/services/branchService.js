...
const createBranch = async (
  ...
  try {
    ...
    context = administrationAudit.createContext({
      ...audit,
      branchId,
...
const updateBranch = async (id, input, audit = {}) => {
  ...
  try {
    ...
    await administrationAudit.branchUpdated({
      context,
      branchId: id,
      before: previous,
      after: branch,
...
const deactivateBranch = async (id, audit = {}) => {
  ...
  try {
    ...
    if (!branch) throw new AppError("Branch not found", 404);
    await administrationAudit.branchDeleted({
      context,
      branch,
...
const getBranchSummary = async (branchId) => {
  const scope = { type: "branch", isGlobal: false, branchId: String(branchId) };
  assertBranchScope(scope);
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const [totalOrders, activeOrders, staffCount] = await Promise.all([
    kitchenRepository.countScoped({ scope, filter: { createdAt: { $gte: today } } }),
    kitchenRepository.countScoped({ scope, filter: { status: { $in: ["pending", "preparing", "ready"] } } }),
