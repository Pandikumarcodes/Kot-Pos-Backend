...
const MODELS = ["gemini-3.1-flash-lite-preview", "gemini-3.1-flash-preview", "gemini-3-flash-preview"];
...
const callGemini = async (prompt) => {
  if (!client) throw new AppError("AI client not initialized — check GEMINI_API_KEY", 503);
  let lastError;
  for (const modelName of MODELS) {
    try {
      const result = await client.models.generateContent({
        ...
        config: {
          systemInstruction: "You are the KOT POS AI analyst for a restaurant. Keep responses short, clear and professional. Max 3 sentences. No markdown formatting.",
        },
      });
...
const chat = async ({ message, context }) => {
  if (!client) throw new AppError("AI service not configured. Add GEMINI_API_KEY to environment.", 503);
  const safeContext = context ? {
    ...
    topItems: Array.isArray(context.topItems) ? context.topItems.slice(0, 5) : [],
...
const getInventoryAlerts = async (branchFilter) => {
  ...
  if (!items.length) {
    return { alerts: [], counts: { critical: 0, warning: 0, info: 0, ok: 0 }, message: "No inventory items found." };
  }
  ...
  const alerts = items.map((item) => {
    ...
    if (currentStock <= 0) {
      ...
      message = daysUntilStockout !== null ? `Stock sufficient for ${daysUntilStockout}+ days.` : "No recent usage data available.";
    }
    return { _id: item._id, name: item.name, currentStock, unit, reorderLevel, avgDailyUsage: parseFloat(avgDailyUsage.toFixed(2)), daysUntilStockout, level, emoji, message };
  });
...
const generateFallbackSummary = (data) => {
  const trend = data.orderTrend === "up" ? "📈 up from previous day" : data.orderTrend === "down" ? "📉 down from previous day" : "stable";
  const stockMessage = data.criticalStockItems.length > 0
...
const getDailySummary = async ({ branchFilter, branchMemberFilter, branchId }) => {
  ...
  const [yesterdayOrders, dayBeforeOrders, yesterdayBills] = await Promise.all([
    ...
    Billing.find({ ...branchMemberFilter, createdAt: { $gte: yesterday, $lt: today }, paymentStatus: "paid" }).lean(),
  ]);
  const totalRevenue = yesterdayBills.reduce((sum, bill) => sum + (bill.totalAmount ?? 0), 0);
  const totalOrders = yesterdayOrders.length;
  const previousOrders = dayBeforeOrders.length;
  const orderChange = previousOrders > 0 ? (((totalOrders - previousOrders) / previousOrders) * 100).toFixed(1) : null;
  const itemCount = {};
  const hourCount = {};
  yesterdayOrders.forEach((order) => {
    order.items?.forEach((item) => { const name = item.name ?? "Unknown"; itemCount[name] = (itemCount[name] ?? 0) + (item.quantity ?? 1); });
    const hour = new Date(order.createdAt).getHours();
    hourCount[hour] = (hourCount[hour] ?? 0) + 1;
  });
  const topItems = Object.entries(itemCount).sort(([, a], [, b]) => b - a).slice(0, 5).map(([name, qty]) => ({ name, qty }));
  const peakEntry = Object.entries(hourCount).sort(([, a], [, b]) => b - a)[0];
  const paymentBreakdown = {};
  yesterdayBills.forEach((bill) => { const method = bill.paymentMethod ?? "unknown"; paymentBreakdown[method] = (paymentBreakdown[method] ?? 0) + 1; });
  ...
  const summaryData = {
    ...
    orderTrend: orderChange ? (parseFloat(orderChange) >= 0 ? "up" : "down") : "neutral",
    topItems,
    peakHour: peakEntry ? `${peakEntry[0]}:00 - ${parseInt(peakEntry[0]) + 1}:00` : "N/A",
    paymentBreakdown,
    dineIn: yesterdayOrders.filter((order) => order.orderType === "dine-in").length,
    takeaway: yesterdayOrders.filter((order) => order.orderType === "takeaway").length,
    avgOrderValue: totalOrders > 0 ? `₹${(totalRevenue / totalOrders).toFixed(0)}` : "₹0",
    criticalStockItems: inventoryItems.filter((item) => (item.currentStock ?? 0) <= (item.reorderLevel ?? 0)).map((item) => item.name),
  };
...
module.exports = { chat, getInventoryAlerts, getDailySummary, generateFallbackSummary };
